...
def centered(img): # 'centering ist ein python eigenname, das Python Modul wird hiermit überschrieben. Das kann zu Problemen führen.'
    centered_img = img - img.mean(axis=(1), keepdims=True) #mean is calculated along 2nd and 3rd axis, meaning the heigth and the width of an image (remember: the first dimension is which picture we are looking at)
    return centered_img

def pca(centered_img, prop_variance): #added input variable centered_img
    #centered_img = np.reshape(centered_img,(-1,1)) #gives array of shape (784,1) #needs to me modifyed
